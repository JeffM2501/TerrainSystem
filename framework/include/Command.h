...
#define DEFEINE_CONTEXT_ITEM(T)\
	...
	public:
		...
		std::string_view GetName() const override { return ActionRegistry::GetAction(ActionHash)->Name; }
	...
	public:
		...
		CommandContainerItem* FindItemByName(std::string_view actionName)
		{
			for (auto& item : Contents)
			{
		...
		CommandContainer* FindContainerByName(std::string_view actionName)
		{
			for (auto& item : Contents)
			{
				if (item->IsContainer() && item->GetName() == actionName)
					return static_cast<CommandContainer*>(item.get());
			}
			return nullptr;
		}
