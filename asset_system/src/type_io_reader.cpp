...
bool TypeReader::AddPrimtiveField(TypeValue* destinationValue, int fieldIndex, PrimitiveFieldInfo* fieldInfo, rapidjson::Value& jsonValue, TypeDatabase& typeDB)
{
...
bool TypeReader::AddPrimtiveListField(TypeValue* destinationValue, int fieldIndex, PrimitiveFieldInfo* fieldInfo, rapidjson::Value& jsonValue, TypeDatabase& typeDB)
{
    if (!jsonValue.IsObject())
        return false;
...
bool TypeReader::ReadTypeListValue(TypeListValue& listValue, rapidjson::Value& jsonValue, TypeDatabase& typeDB)
{
    if (!jsonValue.HasMember(TypeName) || !jsonValue[TypeName].IsString())
        return false;

    TypeInfo* type = typeDB.FindType(jsonValue[TypeName].GetString());
    if (!type)
        return false;
    ...
    if (jsonValue.HasMember(ValueName) && jsonValue[ValueName].IsArray())
    {
        ...
        for (auto& i : array)
        {
            ...
            ReadTypeValue(itemValue, i, typeDB);
        }
    }
...
bool TypeReader::ReadTypeValue(TypeValue* destinationValue, rapidjson::Value& jsonValue, TypeDatabase& typeDB)
{
    if (!jsonValue.HasMember(TypeName) || !jsonValue[TypeName].IsString())
        return false;
        ...
        for (auto& field : jsonValue[Fields].GetObject())
        {
            ...
            if (index >= 0)
            {
                ...
                case FieldType::Primitive:
                    AddPrimtiveField(destinationValue, index, static_cast<PrimitiveFieldInfo*>(fieldInfo), field.value, typeDB);
                    break;

                case FieldType::PrimitiveList:
                    AddPrimtiveListField(destinationValue, index, static_cast<PrimitiveFieldInfo*>(fieldInfo), field.value, typeDB);
                    break;

                case FieldType::Type:
                    ReadTypeValue(destinationValue->GetTypeFieldValue(index), field.value, typeDB);
                    break;

                case FieldType::TypeList:
                    ReadTypeListValue(destinationValue->GetTypeListFieldValue(index), field.value, typeDB);
                    break;
                }
            }
        }
...
bool TypeReader::Read(TypeValue* value, const std::string& fileName, TypeDatabase& typeDB)
{
    ...
    if (RootDocument->HasMember(RootData))
    {
        ...
        return ReadTypeValue(value, rootObject, typeDB);
    }
    return false;
}
