...
namespace Properties
{
	...
	bool ResourceReferenceEditor(Types::TypeValue* value, int fieldIndex, bool expanded)
	{
		...
		if (ImGui::Button(ICON_FA_ELLIPSIS))
		{
			...
			if (fileToOpen)
			{
				AssetManager::FileSystemPath filePath = fileToOpen;
				ref.SetPath(AssetManager::ToAssetPath(filePath));
			}
		}
	...
	bool TextureReferenceEditor(Types::TypeValue* value, int fieldIndex, bool expanded)
	{
		AssetTypes::ResourceReference ref(value->GetTypeFieldValue(fieldIndex));
		...
		return ResourceReferenceEditor(value, fieldIndex, expanded);
	}
